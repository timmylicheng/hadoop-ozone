...
public class TestReconAsPassiveScm {
  ...
  public void testDatanodeRegistrationAndReports() throws Exception {
    ...
    assertTrue(scmContainerManager.listContainers().isEmpty());
  ...
  public void testReconRestart() throws Exception {
    ...
    assertTrue(scmContainerManager.listContainers().isEmpty());
    ContainerManagerV2 reconContainerManager = reconScm.getContainerManager();
    assertTrue(reconContainerManager.listContainers().isEmpty());
    ...
    XceiverClientGrpc client = new XceiverClientGrpc(pipeline, conf);
    runTestOzoneContainerViaDataNode(containerID, client);
    assertFalse(scmContainerManager.listContainers().isEmpty());
